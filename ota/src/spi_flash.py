...
class SPIFlashDevice:
    ...
    def program(self, addr, payload):
        ...
        while programed < length:
            ...
            if data.count(0xFF) != len(data):
                print(
                    f"[{100.0 * programed / length:.2f}%]program 0x{addr + programed:06X}..."
                )
    ...
    class _WriteEnableManager:
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            while (
                0x1 & self.flash_dev.spi.xfer(b"\x05", 1)[0]
            ):  # Status Register-1 S0:WIP
                continue
...
with SPIFlashDevice(0x359F, 0x30F1) as flash:
    # Reset flash with proper delay
    assert flash.reset()
    import time

    time.sleep(0.1)  # 100ms delay after reset
